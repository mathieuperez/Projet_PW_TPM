...
router.patch('/:login/:oldStartDate/:oldAddress', (req, res) => {
    ...
    verifyauth(req, res, token, function () {
        if (renting.country == null || renting.address == null || renting.city  == null || renting.price == null || req.body.startDate == null || renting.time == null || renting.surface == null) {
            res.status(422).json({success: false, message:'Missing Arguments.'});
        }
        else {
...
function verifyDate(req, res, renting, dateProblem, next) {
    Renting.find({"endDate": {"$gte": renting.startDate.getTime()-1, "$lt": renting.endDate.getTime()+1}, "address": renting.address, "login": renting.login}).exec(function(err, rentings){
        if (err) {
            ...
            if (rentings.length == 0) {
                Renting.find({"startDate": {"$gte": renting.startDate.getTime()-1, "$lt": renting.endDate.getTime()+1}, "address": renting.address, "login": renting.login}).exec(function (err, rentings) {
                    if (err) {
